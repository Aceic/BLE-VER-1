...
class apb_item extends ovm_sequence_item;
  `ovm_object_utils(apb_item)
...
class wsh_item extends ovm_sequence_item;
  `ovm_object_utils(wsh_item)
  function new(string name="apb_item");
    super.new(name);
  endfunction
  rand bit unsigned   read;
...
class reg2apb_adapter extends uvm_reg_adapter;
  ...
  `ovm_object_utils(reg2apb_adapter)
...
class my_apb_driver extends any_driver #(apb_item);
   `ovm_component_utils(my_apb_driver)
   function new(string name = "apb_driver", ovm_component parent=null);
...
class blk_env extends ovm_env;

   `ovm_component_utils(blk_env)

   reg_block_B  model;
   apb_agent    apb;
   wsh_agent    wsh;

   function new(string name = "blk_env", ovm_component parent=null);
      super.new(name, parent);
   endfunction: new
