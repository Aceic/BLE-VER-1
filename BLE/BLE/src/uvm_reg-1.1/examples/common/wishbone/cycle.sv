...
class wb_cycle extends ovm_sequence_item;
   ...
   `ovm_object_utils_begin(wb_cycle)
      `ovm_field_enum(cycle_kinds_e, m_kind, OVM_ALL_ON)
      `ovm_field_enum(pipelining_e, m_next_cycle, OVM_ALL_ON)
      `ovm_field_int(m_addr, OVM_ALL_ON)
      `ovm_field_int(m_data, OVM_ALL_ON)
      `ovm_field_int(m_sel, OVM_ALL_ON)
      `ovm_field_int(m_tgc, OVM_ALL_ON)
      `ovm_field_int(m_tga, OVM_ALL_ON)
      `ovm_field_int(m_tgd, OVM_ALL_ON)
      `ovm_field_int(m_lock, OVM_ALL_ON)
      `ovm_field_int(m_n_wss, OVM_ALL_ON)
      `ovm_field_enum(status_e, m_status, OVM_ALL_ON)
...
class wb_reset_seq extends ovm_sequence #(wb_cycle);

   `ovm_object_utils(wb_reset_seq)
...
class reg2wsh_adapter extends uvm_reg_adapter;
  ...
  `ovm_object_utils(reg2wsh_adapter)
  ...
  virtual function void bus2reg(ovm_sequence_item bus_item,
    ...
    if (!$cast(cyc,bus_item)) begin
      `ovm_fatal("NOT_APB_TYPE","Provided bus_item is not of the correct type")
      return;
    end
