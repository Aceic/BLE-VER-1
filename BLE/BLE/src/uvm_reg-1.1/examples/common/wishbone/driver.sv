...
class wb_driver extends ovm_driver #(wb_cycle);
...
task wb_driver::run();
   ...
   forever begin
      ...
      if (tr.m_next_cycle !== wb_cycle::CLASSIC ||
         ...
         `ovm_error("WB_DRIVER", "Only single read/write classic cycles are supported");
      ...
      `ovm_info("WB_DRIVER", {"Transaction Completed:\n", tr.sprint()}, OVM_HIGH);
...
task wb_driver::read(input  bit [63:0] addr,
   ...
   if (status == wb_cycle::TIMEOUT ) begin
      `ovm_error("WB_DRIVER", "Timeout waiting for ACK_I, RTY_I or ERR_I");
   end
...
task wb_driver::reset_wb(int cycles);
   `ovm_info("WB_DRIVER", $sformatf("Resetting Wishbone for %0d cycles...", cycles), OVM_LOW);
