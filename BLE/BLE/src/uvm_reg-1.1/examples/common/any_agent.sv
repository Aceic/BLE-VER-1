...
class any_sequencer #(type REQ=int,RSP=REQ) extends ovm_sequencer #(REQ,RSP);
  `ovm_component_param_utils(any_sequencer #(REQ,RSP))
...
class any_monitor #(type REQ=int,RSP=REQ) extends ovm_component;

  `ovm_component_param_utils(any_monitor #(REQ,RSP))

  ovm_analysis_port #(REQ) req_ap;
  ovm_analysis_port #(RSP) rsp_ap;
...
class any_driver #(type REQ=int,RSP=REQ) extends ovm_component;

  `ovm_component_param_utils(any_driver #(REQ,RSP))
  ...
  ovm_seq_item_pull_port #(REQ) seqr_port;

  ovm_analysis_port #(RSP) rsp_out;
  ...
  task run();
    ...
    `ovm_info({"DRIVER-",get_type_name()},{"Starting in ",mode.name()," mode."},OVM_LOW);
    ...
    forever begin
      ...
      if (!req.read)
        `ovm_info({"DRIVER-",req.get_type_name()},{"Received write request: ",req.convert2string()},OVM_HIGH)
      ...
      if (req.read)
         `ovm_info({"DRIVER-",req.get_type_name()},{"Executed read request: ",req.convert2string()},OVM_HIGH)
      ...
      case (mode)
        ...
        default: begin
          `ovm_error("Unknown mode",{"Internal error: unknown mode ",mode.name(),"'"})
          end
      endcase
...
class any_agent #(type REQ=int, RSP=REQ) extends ovm_component;

  `ovm_component_param_utils(any_agent #(REQ,RSP))
