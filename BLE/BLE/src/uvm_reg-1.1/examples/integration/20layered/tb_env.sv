...
typedef uvm_reg_sequence #(ovm_sequence #(apb_rw)) reg2apb_seq_t;

class tb_env extends ovm_component;

   `ovm_component_utils(tb_env)

   dut_regmodel                 regmodel; 
   ovm_sequencer#(uvm_reg_item) reg_seqr;
...
`endif
   ...
   virtual task run();
     ...
     if (seq == null) begin
       ovm_report_fatal("NO_SEQUENCE","Env's sequence is not defined. Nothing to do. Exiting.");
       return;
     end

     begin : do_reset
       ovm_report_info("RESET","Performing reset of 5 cycles");
       tb_top.rst <= 1;
     ...
     join_none
     ovm_report_info("START_SEQ",{"Starting sequence '",seq.get_name(),"'"});
     seq.model = regmodel;
     seq.start(null);
