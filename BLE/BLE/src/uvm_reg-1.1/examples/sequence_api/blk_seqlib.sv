...
class blk_R_test_seq extends uvm_reg_sequence;
   ...
   `ovm_object_utils(blk_R_test_seq)
   ...
   virtual task body();
      ...
      if (data != rd_data)
        `ovm_error("MISCOMPARE","Unexpected value on read")
      ...
      if (data != rd_data)
        `ovm_error("MISCOMPARE","Unexpected value on peek")

      // Perform a random number of INC operations
      n = ($urandom() % 7) + 3;
      `ovm_info("blk_R_test_seq", $sformatf("Incrementing R %0d times...", n), OVM_NONE);
      repeat (n) begin
      ...
      n = ($urandom() % 8) + 2;
      `ovm_info("blk_R_test_seq", $sformatf("Decrementing R %0d times...", n), OVM_NONE);
      repeat (n) begin
