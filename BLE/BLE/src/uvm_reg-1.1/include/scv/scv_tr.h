...
/*****************************************************************************
  ...
  $Id: //dvt/mti/rel/10.0b/src/scv/scv/scv_tr.h#1 $
...
#if defined(DO_SCV_TR_TRACE) /* DO_SCV_TR_TRACE { */
/* 
 * Debug is sometimes difficult in this code.
 * Sometimes, I find "tracing" more useful.
 *
 * The SCV_TR_TRACE() is useful for sticking into code 
 * streams when debugging on Windows. (Fancy printf,
 * by just adding the line
 *
 *     SCV_TR_TRACE();
 *
 * all over the place in the code where you want to know
 * what is going on.
 *
 * To trace,
 *   1. Uncomment the following line to define the tracing 
 *      routine SCV_TR_TRACE().
 *
 *      #define DO_SCV_TR_TRACE
 *
 *   2. Sprinkle the code to be traced with lines like:
 *
 *      SCV_TR_TRACE();
 *
 *      The more tracing output the better - usually.
 *      See scv_tr.h for details, but effectively, this MACRO will
 *      printf() __FILE__ and __LINE__ and then do fflush(stdout).
 *      The fflush() is important for crashes - and stdout is needed
 *      instead of stderr, since there are other parts of the system
 *      using stdout, and we want our messages intermixed with them.
 */
static void
_scv_tr_trace(char *file, int line)
{
    fprintf(stdout, " -- scv_tr_trace(%s, %d)\n", file, line);
    fflush(stdout);
}
#define SCV_TR_TRACE() _scv_tr_trace(__FILE__, __LINE__)
#else /* } DO_SCV_TR_TRACE { */
#define SCV_TR_TRACE()
#endif /* } DO_SCV_TR_TRACE */
...
//
// Call this function from your sc_main() to use WLF file transaction recording:
//
void scv_tr_wlf_init();
...
typedef mtiLongT scv_tr_relation_handle_t;
...
class scv_tr_db : public scv_object_if {
 ...
 // BEGIN MODELSIM ADDITIONS
 public:
  void mti_set_handle(void *handle);
  void *mti_tr_db_handle;
 // END MODELSIM ADDITIONS
};
...
class scv_tr_stream : public scv_object_if {
 public:
  ...
  scv_tr_stream() { this->mti_tr_stream_handle = 0; this->_scv_tr_stream_core_p = NULL; }
 ...
 // BEGIN MODELSIM ADDITIONS
 public:
  void mti_set_handle(int handle);
  int  mti_tr_stream_handle;
 // END MODELSIM ADDITIONS
};
...
class scv_tr_generator_base : public scv_object_if {
 ...
 public:
  ...
  scv_tr_generator_base() { this->mti_tr_generator_handle = NULL; this->_scv_tr_generator_core_p = NULL; }
