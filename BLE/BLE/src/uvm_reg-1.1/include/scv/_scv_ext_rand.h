...
/*****************************************************************************
  ...
  $Id: //dvt/mti/rel/10.0b/src/scv/scv/_scv_ext_rand.h#1 $
...
// BEGIN MODELSIM MODIFICATIONS [MDE-SCV]
    if (!this->is_shared_vector_elem && this->_has_dynamic_data() && this->_get_dynamic_data()->cdata_) {
...
#ifndef _SCV_INTROSPECTION_ONLY
// BEGIN MODELSIM MODIFICATIONS [MDE-SCV]
    if (!this->is_shared_vector_elem && this->_has_dynamic_data() && 
...
// ----------------------------------------
template<typename T>
class scv_extension_rand< vector<T> >
...
public:
  virtual void next() { GET_RANDOM(); uninitialize(); initialize(); }
  virtual void set_distribution_from(scv_extensions_if* e) {
    for (unsigned i=0; i<this->get_vector_size()->get_unsigned(); ++i) {
      this->_get_array_elt(i)->set_distribution_from(e);
    }
  }
  virtual void disable_randomization() {
  ...
  virtual void get_generator(void) {
    _scv_message::message(_scv_message::INTERNAL_ERROR,"Cannot do get_generator for vector type.");
    return;
  }
  virtual _scv_constraint_data * get_constraint_data() {
    if (!this->_get_dynamic_data()->cdata_)
  ...
  virtual scv_expression form_expression() const {
    _scv_message::message(_scv_message::INTROSPECTION_INVALID_EXPRESSION,"vector",this->get_name());
    return scv_expression();
  }
  virtual void use_constraint(scv_smart_ptr_if& s) {
    _scv_message::message(_scv_message::INTERNAL_ERROR,"Cannot set constraint to vector type.");
  }
  virtual void use_constraint(scv_extensions_if*) {
    _scv_message::message(_scv_message::INTERNAL_ERROR,"Cannot set constraint to vector type.");
  }
...
public: // internal methods
  ...
  virtual void resize_vector(unsigned old_size, unsigned new_size) {
    ...
    // vector<T> extension.
    scv_extension_rw< vector<T> >::resize_vector(old_size, new_size);
  }
};
...
#define _SCV_INTROSPECTION_BASE3 scv_extension_rand< vector<T> >
// END MODELSIM ADDITIONS [MDE-SCV]
