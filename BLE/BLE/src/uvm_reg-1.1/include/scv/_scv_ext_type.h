...
/*****************************************************************************
  ...
  $Id: //dvt/mti/rel/10.0b/src/scv/scv/_scv_ext_type.h#1 $
...
// BEGIN MODELSIM ADDITIONS [MDE-SCV]
  virtual scv_extensions_if *get_vector_size() { return 0; }
  virtual const scv_extensions_if *get_vector_size() const { return 0; }
...
// ----------------------------------------
template<typename T>
class scv_extension_type< vector<T> >
...
public:
  ...
  virtual const char * get_type_name() const {   
    static const char * s = _scv_ext_util_get_name("%s[%u]", 
      scv_extensions<T>().get_type_name(), this->get_vector_size()->get_unsigned());
    return s;
  }

  virtual scv_extension_type_if::data_type get_type() const { return scv_extension_type_if::VECTOR; }
  ...
  virtual int get_array_size() const { return (int)this->get_vector_size()->get_unsigned(); }
  virtual scv_extensions_if * get_array_elt(int i) { return this->_get_array_elt(i); }
  virtual const scv_extensions_if * get_array_elt(int i) const { return this->_get_array_elt(i); }
  ...
  virtual void resize_vector(unsigned old_size, unsigned new_size) {
    // Propagate the effects of resize to the various lower layers of the
    // vector<T> extension.
    scv_extension_util< vector<T> >::resize_vector(old_size, new_size);
  }
};
// END MODELSIM ADDITIONS [MDE-SCV]
...
#ifdef SYSTEMC_H
_SCV_EXT_TYPE_N_FC_D(sc_int<N>,SC_SIGNED);
_SCV_EXT_TYPE_N_FC_D(sc_bigint<N>,SC_SIGNED);
_SCV_EXT_TYPE_N_FC_D(sc_uint<N>,SC_UNSIGNED);
_SCV_EXT_TYPE_N_FC_D(sc_biguint<N>,SC_UNSIGNED);
...
#define _SCV_INTROSPECTION_BASE3 scv_extension_type< vector<T> >
// END MODELSIM ADDITIONS [MDE-SCV]
