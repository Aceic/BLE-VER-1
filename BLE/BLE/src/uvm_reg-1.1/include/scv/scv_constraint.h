...
/*****************************************************************************
  ...
  $Id: //dvt/mti/rel/10.0b/src/scv/scv/scv_constraint.h#1 $
...
public:  
  _scv_hash()
    : _factor(new mtiUlongT * [sizeof(void*)])
    {
      mtiUlongT b = 1;
      for (int i = sizeof(void*)-1; i>=0 ; i--) {
	_factor[i] = new mtiUlongT [256];
	for (int j = 0; j<256; j++) {
	  _factor[i][j] = b*j;
	  b *= 131;
	}
  ...
  mtiUlongT operator()(const void * key) const {
...
//*************************************************************
static bool isPrime(mtiUlongT n) {
  mtiUlongT i = 3;
  if ( n < 3 ) return true;
...
static mtiUlongT nextPrime(mtiUlongT n) {
  while( !isPrime(n) ) n++;
  return n;
}
...
class _scv_open_table {
  ...
  _scv_hash _hash;
  mtiUlongT _numElts;
  mtiUlongT _numBuckets;
  bucketT ** _arrayP;
public:
  _scv_open_table(mtiLongT sizeHint) 
    : _hash(),
    _numElts(0),
    _numBuckets(nextPrime(sizeHint)),
    _arrayP(new bucketT*[_numBuckets]) 
    {
      for (mtiUlongT i=0; i<_numBuckets; ++i) {
	_arrayP[i] = NULL;
      }
    }
  ~_scv_open_table() {
  ...
  inline mtiLongT numElts() const { return _numElts; }
  inline mtiLongT numBuckets() const { return _numBuckets; }
  ...
  inline bool _remove(const void * key) {
    mtiLongT location = _hash(key) % _numBuckets;
    if (_arrayP[location]) {
...
// BEGIN MODELSIM ADDITIONS [MDE-SCV]
  static bddNodeT* get_bdd_ptr(scv_expression e, scv_constraint_base* c, bool hard_constraint);
// BEGIN MODELSIM ADDITIONS [MDE-SCV]
...
private:
  ...
  int distFix;		// enable fix for bad distribution of values when disable_randomization is used
public:
   _scv_constraint_manager();
  ~ _scv_constraint_manager();
public:
...
// BEGIN MODELSIM ADDITIONS [MDE-SCV]
  friend bddNodeT* _scv_bdd_and_ptr(bddNodeT& bh, bddNodeT& bs, const scv_constraint_base* c);
  bddNodeT* get_bdd_ptr(scv_expression e, scv_constraint_base* c, bool hard_constraint);
// END MODELSIM ADDITIONS [MDE-SCV]
...
private:
  ...
  void setExprRep(scv_extensions_if* s, const _scv_expr& e);
...
//  - Used to manage named constraints
////////////////////////////////////////////////////////////////
...
class scv_constraint_expr {
  public:
    ...
    scv_constraint_expr(const char* name, scv_constraint_type type, scv_expression e,
                        const char* file = "unknown", int line = 0);
    scv_constraint_expr() : _is_disabled(true), _type(HARD), _line(0) {}
    scv_constraint_expr& operator=(const scv_constraint_expr& rhs) {
      _is_disabled = rhs._is_disabled;
      _name = rhs._name;
      _type = rhs._type;
      _expr = rhs._expr;
      _file = rhs._file;
    ...
    scv_constraint_type type() const { return _type; }
    const char *file() const { return _file.c_str(); }
    int         line() const { return _line; }
    const scv_expression &expr() const { return _expr; }

    void disable() { _is_disabled = true; }
    void enable()  { _is_disabled = false; }
    bool is_disabled() const { return _is_disabled == true; }

  private:
    bool _is_disabled;
    string _name;
    scv_constraint_type _type;
    scv_expression _expr;
    string _file;
    int _line;
};
// END MODELSIM ADDITIONS [MDE-SCV]
...
// BEGIN MODELSIM ADDITIONS [MDE-SCV]
extern bddNodeT* _scv_bdd_and_ptr(bddNodeT& bh, bddNodeT& bs, const scv_constraint_base* c);
// END MODELSIM ADDITIONS [MDE-SCV]
...
// BEGIN MODELSIM ADDITIONS [MDE-SCV]
  // enable or disable named constraints.
  bool enable_constraint(const char *name) {
    return enable_or_disable_constraint(name, true); }
  bool disable_constraint(const char *name) {
    return enable_or_disable_constraint(name, false); }
  // enable or disable named constraints.
  bool is_defined_constraint(const char *name);
// END MODELSIM ADDITIONS [MDE-SCV]
...
// BEGIN MODELSIM ADDITIONS [MDE-SCV]
  ...
  // return named hard constraints for the class
  virtual scv_expression get_named_constraint() const; 

  // return named soft constraints for the class
  virtual scv_expression get_named_soft_constraint() const; 

  // return unnamed hard constraints for the class
  virtual scv_expression get_unnamed_constraint() const; 

  // return unnamed soft constraints for the class
  virtual scv_expression get_unnamed_soft_constraint() const; 

  void add_named_constraint(const char *name, scv_constraint_expr::scv_constraint_type,
                            scv_expression expr, const char *file, int line);
...
public:
  ...
  void attach_member_ext( scv_smart_ptr_if* new_member );
...
// BEGIN MODELSIM ADDITIONS [MDE-SCV]
  // initialize data for variables in expression
  virtual bool init() = 0;
// END MODELSIM ADDITIONS [MDE-SCV]
...
// BEGIN MODELSIM ADDITIONS [MDE-SCV]
  // named hard constraints for this class
  virtual scv_expression& enh() const; 
  
  // named soft constraints for this class
  virtual scv_expression& ens() const; 

  // named hard constraints for base classes
  virtual scv_expression& enbh() const; 
  
  // named soft constraints for base classes
  virtual scv_expression& enbs() const; 

  // un-ignore soft constraint 
  void un_ignore_soft_constraint(void); 

  // clear expression data 
  void clearExpressionRep(const scv_expression& e);
...
// BEGIN MODELSIM ADDITIONS [MDE-SCV]
  bool enable_or_disable_constraint(const char *name, bool is_enable);
// END MODELSIM ADDITIONS [MDE-SCV]
...
// BEGIN MODELSIM ADDITIONS [MDE-SCV]
  map<string, scv_constraint_expr> _named_constraints;
// END MODELSIM ADDITIONS [MDE-SCV]
...
protected: \
  class_name() { init(); } \
public: \
  class_name(const char* name) { name ? this->name_ = name : this->name_ = "<anonymous>"; \
  _scv_pop_constraint(); init(); init_bdd(); GET_BDD(); this->next();} \
  virtual const char *kind() const { \
...
/* BEGIN MODELSIM MODIFICATIONS [MDE-SCV] */ \
  virtual bddNodeT& get_bdd() { \
    static void* bh = scv_constraint_manager::get_bdd_ptr( \
      class_name::get_constraint(), this, true); \
    static void* bs = scv_constraint_manager::get_bdd_ptr( \
      class_name::get_soft_constraint(), this, false); \
    static void* b = _scv_bdd_and_ptr(*((bddNodeT*)bh), *((bddNodeT*)bs), this); \
    if (this->regenerate_bdd) { \
      bh = scv_constraint_manager::get_bdd_ptr( \
                 class_name::get_constraint(), this, true); \
      bs = scv_constraint_manager::get_bdd_ptr( \
                 class_name::get_soft_constraint(), this, false); \
      b = _scv_bdd_and_ptr(*((bddNodeT*)bh), *((bddNodeT*)bs), this); \
      this->regenerate_bdd = 0; \
    } \
    if (this->ignore_) return *((bddNodeT*)bh); else return *((bddNodeT*)b); } \
/* END MODELSIM MODIFICATIONS [MDE-SCV] */ \
...
/* BEGIN MODELSIM ADDITIONS [MDE-SCV] */ \
  virtual scv_expression& enh() const { static scv_expression e; return e; } \
  virtual scv_expression& ens() const { static scv_expression e; return e; } \
  virtual scv_expression& enbh() const { static scv_expression e; return e; } \
  virtual scv_expression& enbs() const { static scv_expression e; return e; } \
/* END MODELSIM ADDITIONS [MDE-SCV] */ \
  virtual scv_constraint_base* get_copy(scv_constraint_base * from) { \
...
/* BEGIN MODELSIM MODIFICATIONS [MDE-SCV] */ \
  virtual scv_expression get_constraint() const { \
    const scv_expression& unnamed_constr = class_name::get_unnamed_constraint(); \
    const scv_expression& named_constr = class_name::get_named_constraint(); \
    if (named_constr.get_operator() == scv_expression::INT_CONSTANT && named_constr.get_int_value() == 1) \
      return unnamed_constr; \
    return unnamed_constr && named_constr;} \
  virtual scv_expression get_soft_constraint() const { \
    const scv_expression& unnamed_constr = class_name::get_unnamed_soft_constraint(); \
    const scv_expression& named_constr = class_name::get_named_soft_constraint(); \
    if (named_constr.get_operator() == scv_expression::INT_CONSTANT && named_constr.get_int_value() == 1) \
      return unnamed_constr; \
    return unnamed_constr && named_constr;} \
  virtual scv_expression get_unnamed_constraint() const { \
  ...
  virtual scv_expression get_named_constraint() const { \
    const scv_expression& enh_constr = class_name::enh(); \
    const scv_expression& enbh_constr = class_name::enbh(); \
    if (enbh_constr.get_operator() == scv_expression::INT_CONSTANT && enbh_constr.get_int_value() == 1) \
      return enh_constr; \
    else if (enh_constr.get_operator() == scv_expression::INT_CONSTANT && enh_constr.get_int_value() == 1) \
      return enbh_constr; \
    return enh_constr && enbh_constr; } \
  virtual scv_expression get_named_soft_constraint() const { \
    const scv_expression& ens_constr = class_name::ens(); \
    const scv_expression& enbs_constr = class_name::enbs(); \
    if (enbs_constr.get_operator() == scv_expression::INT_CONSTANT && enbs_constr.get_int_value() == 1) \
      return ens_constr; \
    else if (ens_constr.get_operator() == scv_expression::INT_CONSTANT && ens_constr.get_int_value() == 1) \
      return enbs_constr; \
    return ens_constr && enbs_constr; } \
private: \
  virtual bool init() { \
    eh() = es() = ebh() = ebs() = enh() = ens() = enbh() = enbs() = true; \
    this->is_processing_ctor = 1; \
    init_core(); \
    this->is_processing_ctor = 0; \
    this->merge_named_constraints(); \
    return true; \
  } \
/* END MODELSIM MODIFICATIONS [MDE-SCV] */ \
  void init_core() \
...
#define SCV_CONSTRAINT(expr) \
  add_hard_constraint( NULL, expr, __FILE__, __LINE__ )

#define SCV_SOFT_CONSTRAINT(expr) \
  add_soft_constraint( NULL, expr, __FILE__, __LINE__ )

#define SCV_BASE_CONSTRAINT(base_class) \
  ebh() &= base_class::get_unnamed_constraint(); \
  ebs() &= base_class::get_unnamed_soft_constraint(); \
  if (!(base_class::get_named_constraint().get_operator() == scv_expression::INT_CONSTANT && \
        base_class::get_named_constraint().get_int_value() == 1)) \
    enbh() &= base_class::get_named_constraint(); \
  if (!(base_class::get_named_soft_constraint().get_operator() == scv_expression::INT_CONSTANT && \
        base_class::get_named_soft_constraint().get_int_value() == 1)) \
    enbs() &= base_class::get_named_soft_constraint();
// END MODELSIM MODIFICATIONS [MDE-SCV]

// BEGIN MODELSIM ADDITIONS [MDE-SCV]
#define SCV_NAMED_CONSTRAINT(type, name, expr) \
  add_named_constraint(name, type, expr, __FILE__, __LINE__);

// Macros for adding dynamic constraints outside of the constructor
// via a an scv_extensions smart pointer object (scv_smart_ptr<T>)

#define SCV_ADD_HARD_CONSTRAINT(name, expr) \
  add_hard_constraint( name, expr, __FILE__, __LINE__ )

#define SCV_ADD_SOFT_CONSTRAINT(name, expr) \
  add_soft_constraint( name, expr, __FILE__, __LINE__ )

//////////////////////////////////////////////////////////////
// Class : _scv_default_constraint
//   - Defines a default constraint that can be internally attached to
//     scv_extensions<> objects by default.
//
//////////////////////////////////////////////////////////////

class _scv_default_constraint : virtual public scv_constraint_base {
  public:
    SCV_CONSTRAINT_CTOR( _scv_default_constraint ){ }
};
...
// BEGIN MODELSIM ADDITIONS [MDE-SCV]
  static void invalidConstraintName(const char * messageP) {
    _scv_message::message(_scv_message::CONSTRAINT_INVALID_NAME, messageP);
  }
  static void duplicateConstraintName(const char * messageP) {
    _scv_message::message(_scv_message::CONSTRAINT_DUPLICATE_NAME, messageP);
  }
// END MODELSIM ADDITIONS [MDE-SCV]
};
