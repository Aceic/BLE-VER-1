...
/*****************************************************************************
  ...
  $Id: //dvt/mti/rel/10.0b/src/scv/scv/scv_expression.h#1 $
...
public: // get_value methods to access value of constants
  ...
  void get_value(mtiLongT&) const;
  void get_value(mtiUlongT&) const;
...
public: // method for backward compatibility (create_reference)
  template <class T>
  static scv_expression create(sc_core::sc_signal_in_if<T>& s){
    return _scv_create_expression(s);
  }
...
public:
  ...
  virtual void get_value(mtiLongT&) const = 0;
  virtual void get_value(mtiUlongT&) const = 0;
...
public:
  ...
  scv_expression_core(scv_expression::operatorT op,
   ...
   core_(NULL), _data(NULL), _bit_width(0), /* Coverity fix */
...
class scv_expression_core_signal : public scv_expression_core {
  sc_core::sc_signal_in_if<T>*  sig_;
public:
  scv_expression_core_signal(sc_core::sc_signal_in_if<T>& s) : scv_expression_core(scv_expression::SC_SIGNAL) {
...
template <class T>
inline scv_expression _scv_create_expression(sc_core::sc_signal_in_if<T>& s)
{
