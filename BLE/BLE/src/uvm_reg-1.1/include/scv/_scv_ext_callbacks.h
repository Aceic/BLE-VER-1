...
/*****************************************************************************
  ...
  $Id: //dvt/mti/rel/10.0b/src/scv/scv/_scv_ext_callbacks.h#1 $
...
// BEGIN MODELSIM MODIFICATIONS [MDE-SCV]
    if (!this->is_shared_vector_elem && this->_has_dynamic_data() && !this->get_parent())
...
public:
  virtual scv_extensions_if::callback_h _register_cb(scv_extensions_if::callback_base * c) {
	...
	c->set_children(children); /* Coverity fix */
    }
    return next_id;
  }
  virtual void remove_cb(scv_extensions_if::callback_h id) {
...
// ----------------------------------------
template<typename T>
class scv_extension_callbacks< vector<T> >
...
public:
  ...
  virtual scv_extensions_if::callback_h _register_cb(scv_extensions_if::callback_base * c) {
    int next_id = s_add_callback(this->_get_dynamic_data(),c);
    unsigned size = this->get_vector_size()->get_unsigned();
    int * children = new int[size];
  ...
  virtual void resize_vector(unsigned old_size, unsigned new_size) {
    // Propagate the effects of resize to the various lower layers
    // of the vector<T> extension.
    scv_extension_rand< vector<T> >::resize_vector(old_size, new_size);
  }
};
...
#define _SCV_INTROSPECTION_BASE3 scv_extension_callbacks< vector<T> >
// END MODELSIM ADDITIONS [MDE-SCV]
