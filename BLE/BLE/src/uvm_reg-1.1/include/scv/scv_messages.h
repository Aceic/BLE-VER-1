...
/*****************************************************************************
  ...
  $Id: //dvt/mti/rel/10.0b/src/scv/scv/scv_messages.h#1 $
...
_SCV_DEFERR(CONSTRAINT_INVALID_NAME,30,\
           "An invalid name '%s' was used for an attempt to enable or disable a named constraint. ",\
           _scv_message::ERROR,"none")

_SCV_DEFERR(CONSTRAINT_DUPLICATE_NAME,31,\
           "Named constraint with name '%s' already exists. ",\
           _scv_message::ERROR,"none")

_SCV_DEFERR(CMPLX_CONSTRAINT_ON_VECTOR_ELEMENT,32,\
           "Complex constraints cannot be used on elements of a STL vector when the size of the vector is being randomized. ",\
           _scv_message::ERROR,"none")

_SCV_DEFERR(INVALID_ARRAY_SIZE,33,\
           "Array size '%d' exceeds the maximum array size '%d'. ",\
           _scv_message::FATAL,"none")
