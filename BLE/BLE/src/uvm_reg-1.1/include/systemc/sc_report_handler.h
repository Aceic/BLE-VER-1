...
/*****************************************************************************
  ...
  $Id: //dvt/mti/rel/10.0b/src/systemc/sc_report_handler.h#1 $
...
public: // private, actually
    ...
    static sc_msg_def* add_msg_type(int id, const char * msg_type);
...
public:
	...
	//
	// The following are additions from scv_report_handler 
	// since we are replacing it with sc_report_handler
	//

	typedef void (*cb_ptr)(const sc_report *);

	// add callback
    static void register_cb(cb_ptr cb_func);

private:

	static sc_plist<cb_ptr> *cb_list_p;
