...
/*****************************************************************************
  ...
  $Id: //dvt/mti/rel/10.0b/src/systemc/sc_process.h#1 $
...
// BEGIN MODELSIM ADDITIONS
typedef class sc_method_debug_process*  sc_method_debug_handle;
// END MODELSIM ADDITIONS
...
enum sc_curr_proc_kind
{
    ...
    SC_THREAD_PROC_,
    SC_CTHREAD_PROC_,
    SC_METHOD_DEBUG_PROC_
};

// Process flags
const int SC_MTI_PROC_IS_INITIALIZED = 0x01; // Is prepare_for_simulation called on process.
const int SC_MTI_PROC_IS_STACK_SIZE_SET = 0x02; // Whether users made set_stack_size() call
const int SC_MTI_PROC_IS_SC_MAIN = 0x04; // Is the thread for sc_main
...
class sc_process_b : public sc_object {
  ...
  public:
    ...
    enum trigger_t
    {
        ...
        AND_LIST_TIMEOUT,
	// BEGIN MODELSIM ADDITIONS
        DISABLE          /* for thread disable support */
	// END MODELSIM ADDITIONS
    };
  ...
  protected:
    ...
    ptrarray<const sc_event*>    m_static_events;   // Static events waiting on.
...
public:
    ...
    friend void scmti_execute_method_process(void* sc_method);
    friend void scmti_execute_thread_process(void* sc_thread);
    // BEGIN MODELSIM ADDITIONS
    friend void scmti_execute_switch_thread(void* sc_thread, void* current_thread);
    friend void scmti_execute_method_debug_process(void* sc_method_debug);
    // END MODELSIM ADDITIONS

    mtiObjectHandleT mti_get_process_id()
        { return m_mti_process_id; }

    void  mti_create_process_id(sc_process_host* parent);

private:

    mtiObjectHandleT            m_mti_process_id;
