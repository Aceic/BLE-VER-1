...
/*****************************************************************************
  ...
  $Id: //dvt/mti/rel/10.0b/src/systemc/sc_proxy.h#1 $
...
public:
    ...
    X& assign_(mtiUlongT a );

    X& assign_(mtiLongT a );
    ...
    X& operator &= (mtiUlongT b );
    X& operator &= (mtiLongT b );

    X& operator &= ( unsigned int b )
	{ return operator &= ( (mtiUlongT) b ); }

    X& operator &= ( int b )
	{ return operator &= ( (mtiLongT) b ); }
    ...
    const sc_lv_base operator & (mtiUlongT b ) const;
    const sc_lv_base operator & (mtiLongT b ) const;
    ...
    bool operator == (mtiUlongT b ) const;
    bool operator == (mtiLongT b ) const;
	...
	{ return (mtiLongT)to_anything_signed(); }

    mtiUlongT to_ulong() const
	{ return (mtiUlongT)to_anything_unsigned(); }
...
sc_proxy<X>::assign_( int a )
{
	...
	inline
	X&
	sc_proxy<X>::assign_(mtiUlongT a )
	{
		X& x = back_cast();
