// $Id: //dvt/mti/rel/10.0b/src/systemc/bind.hpp#1 $
...
template<class T> bool ref_compare(T const & a, T const & b, mtiLongT)
{
    return a == b;
}
...
template<class F> inline F & unwrap(F * f, mtiLongT)
{
    return *f;
}
...
public:
    ...
    template<class R, class F, class A> R operator()(type<R>, F & f, A &, mtiLongT)
    {
        return unwrap(&f, 0)();
    }

    template<class R, class F, class A> R operator()(type<R>, F const & f, A &, mtiLongT) const
    {
        return unwrap(&f, 0)();
    }
...
public:
    ...
    template<class R, class F, class A> R operator()(type<R>, F & f, A & a, mtiLongT)
    {
        return unwrap(&f, 0)(a[a1_]);
    }

    template<class R, class F, class A> R operator()(type<R>, F const & f, A & a, mtiLongT) const
    {
        return unwrap(&f, 0)(a[a1_]);
    }
