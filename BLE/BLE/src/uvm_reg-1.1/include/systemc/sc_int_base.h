...
/*****************************************************************************
  ...
  $Id: //dvt/mti/rel/10.0b/src/systemc/sc_int_base.h#1 $
...
public:
    ...
    sc_int_subref& operator = (mtiUlongT a )
	{ return operator = ( (int_type) a ); }

    sc_int_subref& operator = (mtiLongT a )
	{ return operator = ( (int_type) a ); }
...
class sc_int_base 
	: public sc_mti_access,		// modelsim additions
...
public:
	...
	: m_val( 0 ), m_len( w ), m_ulen( SC_INTWIDTH - m_len ), m_mti_value(NULL)
	{ check_length(); }

    sc_int_base( int_type v, int w )
	: m_val( v ), m_len( w ), m_ulen( SC_INTWIDTH - m_len ), m_mti_value(NULL)
	{ check_length(); extend_sign(); }

    sc_int_base( const sc_int_base& a )
	: m_val( a.m_val ), m_len( a.m_len ), m_ulen( a.m_ulen ), m_mti_value(NULL)
	{}

    explicit sc_int_base( const sc_int_subref_r& a )
        : m_val( a ), m_len( a.length() ), m_ulen( SC_INTWIDTH - m_len ), m_mti_value(NULL)
        { extend_sign(); }
    ...
    sc_int_base& operator = (mtiUlongT a )
	{ m_val = a; extend_sign(); return *this; }

    sc_int_base& operator = (mtiLongT a )
	{ m_val = a; extend_sign(); return *this; }
    ...
    mtiLongT to_long() const
	{ return (mtiLongT) m_val; }

    mtiUlongT to_ulong() const
	{ return (mtiUlongT) m_val; }
...
#ifndef _32BIT_
    mtiLongT long_low() const
	{ return (mtiLongT) (m_val & UINT64_32ONES); }

    mtiLongT long_high() const
	{ return (mtiLongT) ((m_val >> 32) & UINT64_32ONES); }
#endif
...
{
	mtiLongT result = static_cast<mtiLongT>(operator uint_type());
	return result;
}
...
{
	mtiUlongT result = static_cast<mtiUlongT>(operator uint_type());
	return result;
}
