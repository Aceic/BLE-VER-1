...
/*****************************************************************************
  ...
  $Id: //dvt/mti/rel/10.0b/src/systemc/sc_lv_base.h#1 $
...
public:
	...
	: m_len( 0 ), m_size( 0 ), m_data( 0 ), m_ctrl( 0 ), m_mti_value(NULL)
	{ init( length_ ); }
    ...
    sc_lv_base& operator = (mtiUlongT a )
	{ base_type::assign_( a ); return *this; }

    sc_lv_base& operator = (mtiLongT a )
	{ base_type::assign_( a ); return *this; }
...
// BEGIN MODELSIM MODIFICATIONS
    int wi = DIVIDE_BY_SC_DIGIT_SIZE(i);
    int bi = MODULO_BY_SC_DIGIT_SIZE(i);
...
// BEGIN MODELSIM MODIFICATIONS
    ...
    int bi = MODULO_BY_SC_DIGIT_SIZE(i);
// END MODELSIM MODIFICATIONS
    mtiUlongT mask = SC_DIGIT_ONE << bi;
    m_data[wi] |= mask; // set bit to 1
    m_ctrl[wi] |= mask; // set bit to 1

	// BEGIN MODELSIM MODIFICATIONS
    m_data[wi] &= (mtiUlongT)value << bi | ~mask;
    m_ctrl[wi] &= (mtiUlongT)value >> 1 << bi | ~mask;
	// END MODELSIM MODIFICATIONS
}
...
sc_lv_base::clean_tail()
{
    ...
    mtiUlongT mask = ~SC_DIGIT_ZERO >> (SC_DIGIT_SIZE - bi);
	if ( mask )
	{
		m_data[wi] &= mask;
		m_ctrl[wi] &= mask;
