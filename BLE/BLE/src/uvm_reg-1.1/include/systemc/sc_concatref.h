...
/*****************************************************************************
  ...
  $Id: //dvt/mti/rel/10.0b/src/systemc/sc_concatref.h#1 $
...
public:
    ...
    mtiLongT to_long() const
        { return (mtiLongT)to_int64(); }
    mtiUlongT to_ulong() const
        { return (mtiUlongT)to_uint64(); }
    ...
    inline const sc_concatref& operator = (mtiLongT v )
    {
        m_right_p->concat_set((int64)v, 0);
        m_left_p->concat_set((int64)v, m_len_r);
        return *this;
    ...
    inline const sc_concatref& operator = (mtiUlongT v )
    {
        m_right_p->concat_set((uint64)v, 0);
        m_left_p->concat_set((uint64)v, m_len_r);
        return *this;
...
private:
    ...
    sc_concatref() :
		m_left_p(0), m_right_p(0), m_len(0), m_len_r(0), m_flags(cf_none)
	{}
};
...
inline const sc_unsigned operator << ( 
    const sc_concatref& target, mtiUlongT shift )
{
    return target.value() << (int)shift;
}
...
inline const sc_unsigned operator << ( const sc_concatref& target, mtiLongT shift )
{
    return target.value() << (int)shift;
}
...
#define SC_CONCAT_OP(RESULT,OP) \
    ...
    SC_CONCAT_OP_TYPE(const sc_signed,OP,mtiLongT) \
    SC_CONCAT_OP_TYPE(const sc_signed,OP,int64) \
    SC_CONCAT_OP_TYPE(RESULT,OP,unsigned int) \
    SC_CONCAT_OP_TYPE(RESULT,OP,mtiUlongT) \
...
#define SC_CONCAT_BOOL_OP(OP) \
	...
	SC_CONCAT_OP_TYPE(bool,OP,mtiLongT) \
	SC_CONCAT_OP_TYPE(bool,OP,int64) \
	SC_CONCAT_OP_TYPE(bool,OP,unsigned int) \
	SC_CONCAT_OP_TYPE(bool,OP,mtiUlongT) \
